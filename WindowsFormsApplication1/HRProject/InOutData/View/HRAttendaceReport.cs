...
namespace WindowsFormsApplication1.HRProject.InOutData.View
{
 ...
 | System.Windows.Forms.AnchorStyles.Right)));
        ...
        //Vũ update 15/1/2022
        private void Btn_timekeepingList_Click(object sender, EventArgs e)
        {
            System.Windows.Input.Cursor oldCursor = Mouse.OverrideCursor;
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    int IDSession = GetSessionID.GetsessionID(dtpk_choose.Value);
                    List<EmployeeTimekeeping> employeeTimekeepings = getdataInout.GetEmployeeTimekeepings(dtpk_choose.Value, IDSession);
                    ...
                    saveFileDialog.RestoreDirectory = true;
                    hRReport.ExportExcelTimekeepingReport(pathsave, employeeTimekeepings, dtpk_choose.Value);
                    var resultMessage = MessageBox.Show("Timekeeping Daily Report export to excel sucessful ! \n\r Do you want to open this file ?", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                    if (resultMessage == DialogResult.Yes)
                    {
